...
def migrate(cr, version):
    """Fix initial_date calculation in action_get_devices.

    The previous code added timedelta(days=1) to the last reading time before
    truncating to a date. This caused readings from the rest of that same day
    to be permanently skipped whenever the sync did not complete until midnight.

    Fix: use the date of the last reading directly (no +1 day). The upsert
    handles duplicate rows harmlessly.
    """
    env = api.Environment(cr, SUPERUSER_ID, {})

    try:
        action = env.ref(
            'remotecontrol_alsambo.remotecontrol_alsambo_action_get_devices'
        )
        action.write({
            'code': '''
...
for sensor in sensors:
    ...
    if last_reading_time:
        try:
            # Use the same date as the last reading (not +1 day) to avoid
            # skipping readings from the rest of that day. Upsert handles
