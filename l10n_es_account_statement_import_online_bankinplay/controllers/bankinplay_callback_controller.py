...
class BankinplayCallbackController(http.Controller):
    ...
    def handle_bankinplay_callback(self, **kw):
        ...
        # handled yet
        if (
            "responseId" in json_data
            and "signature" in json_data
            and "triggered_event" in json_data
            and json_data["triggered_event"] == "lectura_cierre"
        ):
            import_statement = (
            ...
            if import_statement:
                import_statement.journal_id.online_bank_statement_provider_id._bankinplay_update_statement_data_after_callback(
                ...
                if remote_response:
                    ...
                    json_data["data"] = interface_model._decrypt_bankinplay_data(
                        json_data["data"], bankinplay_api_key, bankinplay_api_secret
                    )
                    url = (
                        remote_response.endpoint_return_url
                        + "/webhook/bankinplay_callback"
                    )
                    requests.post(
                        url,
