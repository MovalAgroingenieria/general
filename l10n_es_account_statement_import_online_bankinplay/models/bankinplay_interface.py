...
class BankinplayInterface(models.AbstractModel):
    ...
    def _set_close_movements_callback(self, access_data, date_since, date_until):
    ...
    def _post_request(self, access_data, url, params):
        ...
        _logger.debug(
            _("POST request to %s with headers %s and params %s"), url, params, headers
        )
        response = requests.post(url, params=params, headers=headers)
        response_parsed = self._get_response_data(response)
        # Check if response is cipher for V2 of the API
        if response_parsed.get("signature", False) and response_parsed.get(
            "data", False
        ):
    ...
    def _get_response_data(self, response):
        ...
        _logger.debug(_("HTTP answer code %s from Bankinplay"), response.status_code)
