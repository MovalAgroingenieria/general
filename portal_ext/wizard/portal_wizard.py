# -*- coding: utf-8 -*-
# 2023 Moval Agroingeniería
# License AGPL-3 - See http://www.gnu.org/licenses/agpl-3.0.html

from lxml import etree

from odoo.exceptions import UserError
from odoo import api, fields, models, _


class PortalWizard(models.TransientModel):
    _inherit = 'portal.wizard'

    send_email = fields.Boolean(
        string="Send email",
        default=True,
        help="If checked, an invitation email will be sent.")

    portal_id = fields.Many2one(
        'res.groups',
        domain=[('is_portal', '=', True)],
        required=True,
        string='Portal',
        default=lambda self: self.env.ref(
            'base.group_portal', raise_if_not_found=False),
        help="The portal that users can be added in or removed from.")

    @api.model
    def fields_view_get(self, view_id=None, view_type='form',
                        toolbar=False, submenu=False):
        # The 'portal_id' field lists every group flagged as 'is_portal'. In
        # this platform several groups are portal groups (WUA portal user,
        # edition mode, etc.), so the selection must be restricted to the
        # standard 'Portal' group only.
        result = super(PortalWizard, self).fields_view_get(
            view_id=view_id, view_type=view_type, toolbar=toolbar,
            submenu=submenu)
        portal_group = self.env.ref(
            'base.group_portal', raise_if_not_found=False)
        if portal_group and view_type == 'form':
            doc = etree.XML(result['arch'])
            for node in doc.xpath("//field[@name='portal_id']"):
                node.set('domain', "[('id', '=', %d)]" % portal_group.id)
            result['arch'] = etree.tostring(doc)
        return result

    @api.onchange('portal_id')
    def onchange_portal_id(self):
        partner_child_portal = self.env['ir.values'].sudo().get_default(
            'base.config.settings',
            'partner_child_portal'
        )
        portal_login_from_vat = self.env['ir.values'].sudo().get_default(
            'base.config.settings',
            'portal_login_from_vat'
        )
        alias_domain = self.env['ir.config_parameter'].sudo().get_param(
            'mail.catchall.domain',
            default=''
        )
        if not partner_child_portal:
            partner_ids = self.env.context.get('active_ids', [])
            contact_ids = set()
            user_changes = []

            for partner in self.env['res.partner'].sudo().browse(partner_ids):
                if partner.id in contact_ids:
                    continue
                contact_ids.add(partner.id)

                autogenerated = False
                in_portal = True

                # If the contact has a real email it is used as portal login.
                # Otherwise the login is autogenerated: from the NIF/DNI (with
                # the check letter in lowercase) when the
                # 'portal_login_from_vat' setting is enabled, or from the
                # (zero-padded) partner code.
                if partner.email:
                    email = partner.email
                else:
                    autogenerated = True
                    if portal_login_from_vat and partner.vat:
                        local_part = partner.vat[2:].lower()
                    else:
                        local_part = str(partner.partner_code or '').zfill(4)
                    email = '%s@%s' % (local_part, alias_domain)

                if partner.user_ids:
                    in_portal = self.portal_id in partner.user_ids[0].groups_id

                user_changes.append((0, 0, {
                    'partner_id': partner.id,
                    'email': email,
                    'in_portal': in_portal,
                    'autogenerated': autogenerated,
                }))

            self.user_ids = user_changes
        if partner_child_portal:
            partner_ids = self.env.context.get('active_ids', [])
            contact_ids = set()
            user_changes = []
            for partner in self.env['res.partner'].sudo().browse(partner_ids):
                contact_partners = partner.child_ids or [partner]
                for contact in contact_partners:
                    if contact.id not in contact_ids:
                        contact_ids.add(contact.id)
                        in_portal = False
                        if contact.user_ids:
                            in_portal = \
                                self.portal_id in contact.user_ids[0].groups_id
                        user_changes.append((0, 0, {
                            'partner_id': contact.id,
                            'email': contact.email,
                            'in_portal': in_portal,
                        }))
            self.user_ids = user_changes


class PortalWizardUser(models.TransientModel):
    _inherit = 'portal.wizard.user'

    autogenerated = fields.Boolean(string='Autogenerated', default=False)

    @api.multi
    def action_apply(self):
        self.env['res.partner'].check_access_rights('write')
        error_msg = self.get_error_messages()
        if error_msg:
            raise UserError("\n\n".join(error_msg))
        for wizard_user in self.sudo().with_context(active_test=False):
            group_portal = wizard_user.wizard_id.portal_id
            if not group_portal.is_portal:
                raise UserError(
                    _('Group %s is not a portal') % group_portal.name)
            user = wizard_user.partner_id.user_ids[0] \
                if wizard_user.partner_id.user_ids else None
            if wizard_user.partner_id.email != wizard_user.email:
                wizard_user.partner_id.write({'email': wizard_user.email})
            if wizard_user.in_portal:
                user_portal = None
                if not user:
                    if wizard_user.partner_id.company_id:
                        company_id = wizard_user.partner_id.company_id.id
                    else:
                        company_id = self.env['res.company'].\
                            _company_default_get('res.users').id
                    user_portal = wizard_user.sudo().with_context(
                        company_id=company_id)._create_user()
                else:
                    user_portal = user
                wizard_user.write({'user_id': user_portal.id})
                if wizard_user.autogenerated:
                    if wizard_user.partner_id.vat:
                        password = wizard_user.partner_id.vat[2:]
                        wizard_user.user_id.write({'password': password})

                if (not wizard_user.user_id.active or group_portal not in
                        wizard_user.user_id.groups_id):
                    wizard_user.user_id.write(
                        {'active': True,
                         'groups_id': [(4, group_portal.id)]})
                    wizard_user.user_id.partner_id.signup_prepare()
                    if wizard_user.wizard_id.send_email:
                        wizard_user.with_context(
                            active_test=True)._send_email()
                wizard_user.refresh()
            else:
                if user and group_portal in user.groups_id:
                    if len(user.groups_id) <= 1:
                        user.write({'groups_id': [(3, group_portal.id)],
                                    'active': False})
                    else:
                        user.write({'groups_id': [(3, group_portal.id)]})
