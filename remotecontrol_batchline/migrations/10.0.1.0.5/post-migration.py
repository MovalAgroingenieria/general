...
def migrate(cr, version):
    ...
    # Fix "Get devices (plan)" and "Get pressure devices (plan)" actions:
    # remove +timedelta(days=1) from initial_date calculation.
    # Safe because upsert handles duplicates.

    # ── 1) Fix sensors plan action ────────────────────────────────────
    new_code_sensors = """\
...
for sensor in sensors:
    ...
    try:
        ...
        sensor_cfg = {}
    sensor_tag = (sensor_cfg.get('tag') or '').strip()
    if sensor_tag:
        try:
            device_cfg = json.loads(device.remotecontrol_params or '{}') or {}
        except Exception:
            device_cfg = {}
        device_id = (device_cfg.get('device_id') or '').strip()
        if device_id:
...
"""
    ...
    # ── 2) Fix pressure sensors plan action ───────────────────────────
    new_code_pressure = """\
# Build pressure sensors plan
Remote = self.remote_id
Sensor = env['mdm.measurement.device.sensor']
Reading = env['mdm.measurement.device.sensor.reading']
today_str = fields.Date.today()
...
"""
    ...
    xmlid1 = '%s.remotecontrol_batchline_action_get_devices' % MODULE
    action1 = env.ref(xmlid1, raise_if_not_found=False)
    if action1:
        action1.write({'code': new_code_sensors})

    xmlid2 = '%s.remotecontrol_batchline_action_get_pressure_devices' % MODULE
    action2 = env.ref(xmlid2, raise_if_not_found=False)
    if action2:
        action2.write({'code': new_code_pressure})
