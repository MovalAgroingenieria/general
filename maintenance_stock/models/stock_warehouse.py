...
class StockWarehouse(models.Model):
    ...
    def _create_or_update_sequences_and_picking_types(self):
        warehouse_data = \
            super(StockWarehouse,
                  self)._create_or_update_sequences_and_picking_types()
    ...
    def _get_picking_type_create_values(self, max_sequence):
        data, max_sequence_new = \
            super(StockWarehouse, self)._get_picking_type_create_values(max_sequence)
        return {
            #**data,
            'cons_type_id': {
                'name': _('Consumption'),
    ...
    def _get_sequence_values(self):
        data = super(StockWarehouse, self)._get_sequence_values()
        return {
            #**data,
            'cons_type_id': {
    ...
    def _get_locations_values(self, vals):
        sub_locations = super(StockWarehouse, self)._get_locations_values(vals)
