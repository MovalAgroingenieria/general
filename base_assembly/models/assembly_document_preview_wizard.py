...
class AssemblyDocumentPreviewWizard(
    ...
    def _preview_inner_html_and_kind(self):
        ...
        if not asm or not doc:
            return "", "default"
        if doc == "publication":
            inner, src = asm._get_rendered_publication_parts()
        elif doc == "delegation_body":
            inner, src = asm._get_rendered_delegation_document_parts()
        elif doc == "delegation_footer":
            inner, src = asm._get_rendered_delegation_footer_parts()
        elif doc == "delegation_combined":
            body, bs = asm._get_rendered_delegation_document_parts()
            foot, fs = asm._get_rendered_delegation_footer_parts()
            bundle = (
            ...
            if is_html_empty(str(bundle)):
                inner = (
                    Markup('<div class="o_assembly_af_delegation_bundle">')
                    + asm._assembly_render_minimal_title_html()
                    + Markup("</div>")
                )
            else:
                inner = bundle
            kind = asm._preview_template_kind_from_sources(bs, fs)
            return str(inner), kind
        elif doc == "ballot_intro":
            inner, src = asm._get_rendered_ballot_intro_parts()
        else:
            inner, src = asm._get_rendered_ballot_nominative_intro_parts()
    ...
    def _wrap_preview_sheet(self, inner_html, label_text, template_kind):
        ...
        out = (
            ...
            % (
                ...
                escape(second),
                escape(self.env._("Session start:")),
                escape(session),
    ...
    def _preview_payload_write_values(self):
        ...
        asm = self.assembly_id
        label = asm._preview_badge_label_from_kind(kind) if asm else ""
        return {
