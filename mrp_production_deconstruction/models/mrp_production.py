...
class MrpProduction(models.Model):
    ...
    def write(self, vals):
        ...
        if 'move_raw_ids' in vals:
            for line in self:
                if line.is_deconstruction and line.move_raw_ids:
                    for move in line.move_raw_ids:
                        move.location_id = line.location_src_id.id
                        move.location_dest_id = line.production_location_id.id
        return res
        ...
        for production in self:
            if production.bom_id and (
                ...
                if dest.usage == 'production':
                    old_qtys = {x.id: x.product_uom_qty for x in
                                   production.move_raw_ids}
                    # When this happens, the location_src_id is used
                    # on move raw quantity calculus, so store first and then
                    # update the quantity (Crappy)
                    production.write({
                    ...
                    for line in production.move_raw_ids:
                        line.write({
                            ...
                            'product_uom_qty': old_qtys[line.id]})
