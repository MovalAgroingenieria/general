    ...
    function pushState(cStack = controllerStack) {
        ...
        const actions = cStack.map((controller) => {
            ...
            if (action.path || action.id) {
                ...
                // Include model in URL for ir.actions.act_window (e.g. ?res_model=res.partner)
                if (action.type === "ir.actions.act_window" && props.resModel) {
                    actionState.res_model = props.resModel;
                }
