...
class SmsApi(models.AbstractModel):
    ...
    def _send_sms(self, numbers, message):
        ...
        sms_service_set = (
            ...
            .sudo()
            .get_param("sms_alternatives.sms_service")
        )
    ...
    def _contact_wausms(self, send_mode, params):
        ...
        service_url = (
            self.env["ir.config_parameter"]
            .sudo()
            .get_param("sms_wausms.wausms_service_url")
        )
        service_user = (
            self.env["ir.config_parameter"]
            .sudo()
            .get_param("sms_wausms.wausms_service_api_user")
        )
        service_passwd = (
            self.env["ir.config_parameter"]
            .sudo()
            .get_param("sms_wausms.wausms_service_api_passwd")
        )
        sender = (
            self.env["ir.config_parameter"]
            .sudo()
            .get_param("sms_wausms.wausms_service_sender")
        )
        if not service_url or not service_user or not service_passwd or not sender:
            raise exceptions.ValidationError(
        ...
        if send_mode == "single":
            response = self.wausms_response_single(
                service_url=service_url, sender=sender, headers=headers, params=params
            )
    ...
    def wausms_response_batch(self, service_url, sender, headers, params):
        ...
        for message in params["messages"]:
            ...
            try:
                response_raw = requests.post(service_url, headers=headers, data=data)
                state = self._check_sms_state(response_raw.status_code)
                if "error" in response_raw.text:
                    wausms_response = json.loads(response_raw.text)["error"][
                        "description"
                    ]
                else:
            ...
            if response:
                ...
                current_sms.write({"wausms_response": wausms_response})
